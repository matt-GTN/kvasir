...
# Tools
from tools import google_search_tool, parse_linkedin_title, scrape_webpage_tool
...
def parse_results_node(state: AgentState):
    """
    Takes the raw_search_results, simplifies them, and uses an LLM
...
def personalization_node(state: AgentState):
    ...
    for prospect in prospects:
        print(f"--- Researching {prospect['name']} ---")
        # 1. Scrape their URL for content
        content = scrape_webpage_tool.invoke({"url": prospect['url']})
        
        # 2. Call the LLM with all the context
        response = llm.invoke(
        ...
        llm_output = remove_json_blocks(response)
...
workflow.add_edge("filter_search_results_node", "parse_results_node")
workflow.add_edge("parse_results_node", "deduplicate_prospects_node")
